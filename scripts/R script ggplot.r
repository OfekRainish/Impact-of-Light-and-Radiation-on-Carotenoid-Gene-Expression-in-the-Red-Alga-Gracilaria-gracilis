...
ggplot(pi_data, aes(x=Group, y=Am))+
  geom_point(aes(color=Group, shape=Group), size=3)+
  scale_color_manual(values=c("darkmagenta", "gold"))
...
setwd("C:/Users/USER/OneDrive/Desktop/r") # set working directory
...
#change y axis to wanted organism everywhere!
library(ggplot2)
...
#1. test homogeneity of variance (overall)
...
#the function: for each pair, is it significant and what type of correlation
...
#maybe try correlating live cov with something else that is interesting
str(Photosurvey)

ggplot(Photosurvey, aes(x=Porifera, y=Chordata))+
  geom_point()+
  geom_smooth(method='lm')+
  theme_light()
correlation <- cor(Photosurvey$Porifera, Photosurvey$Chordata)
print(correlation)
